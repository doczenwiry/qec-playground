...
def simulate(
    ...
    tasks = [
        sinter.Task(
            ...
            postselection_mask=np.packbits(np.ones(circuitry.num_detectors, dtype=bool))
            if postselection
            else None,
    ...
    error_rates.loglog()
    error_rates.set_title("Error rates")
    ...
    if postselection:
        ...
        discard_rates.set_title("Discard rates")
...
def sample(
    ...
    for container in ax.containers:
        ax.bar_label(
            ...
            fmt=lambda x: f"{float(100.0 * x / shots):.2f}%",
            fontsize=fontsize,
        )
