...
def convert_to_pauli(
    stabilizer: str, support: list[int], qubits: int
) -> stim.PauliString:
    pauli = ""
                ...
                for tgt in instruction.targets_copy():
                    if tgt.is_qubit_target:
                        opcode = instruction.name + (
                            "Z" if instruction.name in ("M", "MR") else ""
                        )
...
def check_pauli_flow(
    ...
    pauliI: stim.PauliString,
    pauliO: stim.PauliString,
    solution: Optional[list[int]] = None,
) -> tuple[bool, list[int]]:
    if solution is None:
        solution = (
            circuit.solve_flow_measurements([stim.Flow(input=pauliI, output=pauliO)])[0]
            or []
        )
...
def check_flow_preservation(
    ...
    measurements_index: list[str] = extract_measurements(circuit) or list()
    solution = None
    if measurements is not None:
        solution = list(map(lambda m: measurements_index.index(m), measurements))

    pauli_string = convert_to_pauli(pauli, support, qubits=circuit.num_qubits)
    preserved, solution = check_pauli_flow(
        circuit, pauliI=pauli_string, pauliO=pauli_string, solution=solution
    )
    if len(solution) > 0:
        inclusion = (
            f"(incl. meas. {list(map(lambda m: measurements_index[m], solution))})"
        )
    else:
        inclusion = ""
...
def check_state_preparation(
    ...
    solution = None
    if measurements is not None:
        solution = list(map(measurements_index.index, measurements))

    identity = stim.PauliString(circuit.num_qubits)
    pauli_string = convert_to_pauli(pauli, support, qubits=circuit.num_qubits)
    preserved, solution = check_pauli_flow(
        circuit, pauliI=identity, pauliO=pauli_string, solution=solution
    )
    if len(solution) > 0:
        inclusion = (
            f"(incl. meas. {list(map(lambda m: measurements_index[m], solution))})"
...
def check_syndrome_extraction(
    ...
    preserved, solution = check_pauli_flow(
        circuit, pauliI=pauli, pauliO=identity, solution=solution
    )
    report = (
        f"Extracted (incl. meas. {
            list(map(lambda m: measurements_index[m], solution))
        })"
        if preserved
        else "DISTORTED"
    )
