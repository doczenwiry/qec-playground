...
class ExpandingSurfaceCodePatch:
    ...
    CORNERS: Final[list[tuple[float, float]]] = [
        (-0.5, -0.5),
        (+0.5, -0.5),
        (+0.5, +0.5),
        (-0.5, +0.5),
    ...
    def __init__(
        ...
        self.qubits: dict[str, dict[int, tuple[tuple[float, float], int]]] = dict()

        ax, ay = anchor
        data_qubits: dict[int, tuple[tuple[float, float], int]] = {
        ...
        for qi in range(ancilla_count):
            location = (
                ax + 0.5 + 2 * (qi % width) - ((qi // width) % 2),
                ay + 0.5 + (qi // width),
            )
            z_ancilla[array.qubits[location]] = (location, qi)
        self.qubits["Z"] = z_ancilla
        ...
        self.qubits["X"] = x_ancilla

    @property
    def num_qubits(self):
        return sum(len(self.qubits[qtype]) for qtype in ["X", "Z", "D"])
        ...
        match basis:
            ...
            case Pauli.Y:
                ...
                for i in range(self.__distance + self.__expansion):
                    if i == cross:
                        logical[
                            self.__physical_qubits.qubits[(ax + cross, ay + cross)]
                        ] = "Y"
                    else:
                        logical[self.__physical_qubits.qubits[(ax + cross, ay + i)]] = (
                            "X"
                        )
                        logical[self.__physical_qubits.qubits[(ax + i, ay + cross)]] = (
                            "Z"
                        )
                return logical
            case Pauli.Z:
    ...
    def active_qubits(self, qtype: str, expanded: bool = False):
        return iter(
            dq for dq in self.qubits[qtype] if self.is_qubit_active(dq, expanded)
        )
    ...
    def is_qubit_active(self, qubit: int, expanded: bool) -> bool:
        ...
        if qubit in self.qubits["D"]:
            (px, py), _ = self.qubits["D"][qubit]
            return lower_x - 0.5 <= px <= upper_x and lower_y - 0.5 <= py <= upper_y
        elif qubit in self.qubits["Z"]:
            (px, py), _ = self.qubits["Z"][qubit]
            return lower_x - 0.5 <= px <= upper_x and lower_y + 0.5 <= py <= upper_y
        elif qubit in self.qubits["X"]:
            (px, py), _ = self.qubits["X"][qubit]
            return lower_x + 0.5 <= px <= upper_x and lower_y - 0.5 <= py <= upper_y
    ...
    def qubit_in_expansion(self, qubit: int) -> bool:
        ...
        repository = (
            ...
            if qubit in self.qubits["Z"]
            else "X"
        )
        (px, py), _ = self.qubits[repository][qubit]
        return not (ax + self.__expansion <= px and ay + self.__expansion <= py)

    def __get_polygon(self, px, py, expanded: bool = False):
        return [
            self.__physical_qubits.qubits[px + dx, py + dy]
            for dx, dy in ExpandingSurfaceCodePatch.CORNERS
            if self.is_qubit_active(
                self.__physical_qubits.qubits[px + dx, py + dy], expanded
            )
        ]
    ...
    def get_polygons(self, expanded: bool = False, opacity: float = 0.5):
        ...
        for stabilizer in ["X", "Z"]:
            for (px, py), _ in iter(
                zl
                for zq, zl in self.qubits[stabilizer].items()
                if self.is_qubit_active(zq, expanded)
            ):
    ...
    def annotate_detectors(
        ...
        for stabilizer in ["X", "Z"]:
            for qi, qa in enumerate(self.qubits[stabilizer]):
                if not self.qubit_in_expansion(qa):
                    circuitry.annotate_detector(
                        f"{prefix}:{stabilizer}{qi}",
                        f"{prefix}:{stabilizer}{source.get_qubit_index(stabilizer, qa)}",
                    )
                else:  # self.qubit_in_expansion(qa)
                    ax, ay = self.anchor
                    (px, py), _ = self.qubits[stabilizer][qa]
    ...
    def append_expansion(self, circuit: Circuitry, moment: int, prefix: str = ""):
        match moment:
            case 0:
                ...
                for stabilizer in ["X", "Z"]:
                    circuit.append(
                        f"R{stabilizer}", self.active_qubits(stabilizer, True)
                    )
                ...
                ax, ay = self.anchor
                for dq in filter(
                    self.qubit_in_expansion, self.active_qubits("D", True)
                ):
                    (px, py), _ = self.qubits["D"][dq]
