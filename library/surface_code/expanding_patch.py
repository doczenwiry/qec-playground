...
class ExpandingSurfaceCodePatch:
    ...
    def __init__(
        ...
        self.__qubits: dict[str, dict[int, tuple[tuple[float, float], int]]] = dict()
    ...
    def active_qubits(self, qtype: str, expanded: bool = False):
        return iter(
            dq for dq in self.__qubits[qtype] if self.is_qubit_active(dq, expanded)
        )
    ...
    def get_polygons(self, expanded: bool = False, opacity: float = 0.5):
        ...
        for stabilizer in ["X", "Z"]:
            for (px, py), _ in iter(
                zl
                for zq, zl in self.__qubits[stabilizer].items()
            ...
            for qi, qa in enumerate(self.__qubits[stabilizer]):
                if not self.qubit_in_expansion(qa):
                    circuitry.annotate_detector(
                        ...
                        f"SC{last}:{stabilizer}{source.get_qubit_index(stabilizer, qa)}",
                    )
