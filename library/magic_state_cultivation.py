...
class MagicStateCultivation:
    ...
    def __init__(
        ...
        self.__inactive_source = lambda location: location[1] == ay + target_distance - 5.5
        self.__junction = JunctionPatch(self.qubits, anchor=(ax + target_distance - 6, ay + target_distance - 6))
    ...
    def get_polygons(self, opacity: float = 0.5):
        ...
        for ancilla in range(self.__target_distance):
            ...
            polygon = [self.qubits[px + ancilla + dx, py + dy] for dx, dy in MagicStateCultivation.CORNERS if sx <= px + ancilla + dx]
            polygons.append(f"POLYGON({x},0,{z},{opacity}) {" ".join(map(str, polygon))}")
            if ancilla % 2 == 0 and ancilla > 0:
                polygon = [self.qubits[px + ancilla + dx, py + dy] for dx, dy in [(-0.5, +0.5), (+0.5, +0.5)] if
                           sx <= px + ancilla + dx]
        ...
        for ancilla in range(self.__target_distance):
            px, py = sx + 0.5, sy - 0.5
            x, z = int(ancilla % 2 == 1), int(ancilla % 2 == 0)
            polygon = [self.qubits[px + ancilla + dx, py + dy] for dx, dy in MagicStateCultivation.CORNERS if px + ancilla + dx < sx + self.__target_distance]
            polygons.append(f"POLYGON({x},0,{z},{opacity}) {" ".join(map(str, polygon))}")
        ...
        for ancilla in range(self.__target_distance):
            ...
            polygon = [self.qubits[px + dx, py + ancilla + dy] for dx, dy in MagicStateCultivation.CORNERS if py + ancilla + dy < sy + self.__target_distance]
            polygons.append(f"POLYGON({x},0,{z},{opacity}) {" ".join(map(str, polygon))}")
            if ancilla % 2 == 0 and ancilla < self.__target_distance - 1:
                polygon = [self.qubits[px + dx, py + ancilla + dy] for dx, dy in [(+0.5, -0.5), (+0.5, +0.5)] if
                           sy <= py + ancilla + dy]
        ...
        for ancilla in range(self.__target_distance):
            px, py = sx + 0.5, sy - 0.5
            x, z = int(ancilla % 2 == 0), int(ancilla % 2 == 1)
            polygon = [self.qubits[px + dx, py + ancilla + dy] for dx, dy in MagicStateCultivation.CORNERS if sy <= py + ancilla + dy]
            polygons.append(f"POLYGON({x},0,{z},{opacity}) {" ".join(map(str, polygon))}")
    ...
    def append_preparation(self):
        self.circuitry.annotate_polygons(self.target.get_polygons(expanded=True, opacity=MagicStateCultivation.EXPANDED_OPACITY))
        self.circuitry.annotate_polygons(self.steane.get_polygons(initial=True))
        if self.__draw_neighbors:
            self.circuitry.annotate_polygons(self.get_polygons(opacity=MagicStateCultivation.EXPANDED_OPACITY))
    ...
    def append_teleportation(self, rounds: int = 3):
        ...
        self.circuitry.annotate_polygons(self.source.get_polygons(self.__inactive_source))
        if self.__draw_neighbors:
            self.circuitry.annotate_polygons(self.get_polygons(opacity=MagicStateCultivation.EXPANDED_OPACITY))

        for rnd in range(rounds):
            for mmt in self.steane.TELEPORTATION_MOMENTS:
                self.steane.append_teleportation(self.circuitry, moment=mmt, prefix=f"TPT{rnd}")
                self.__junction.append_syndrome(self.circuitry, moment=mmt, prefix=f"JCT{rnd}")
                self.source.append_round(
                    self.circuitry, moment=mmt, prepare=Pauli.X if rnd == 0 else None, prefix=f"SC{rnd}",
                    inactive=self.__inactive_source
                )
                self.circuitry.append_tick()

        self.circuitry.annotate_polygons(self.target.get_polygons(expanded=True, opacity=MagicStateCultivation.EXPANDED_OPACITY))
        self.circuitry.annotate_polygons(self.steane.get_polygons(opacity=2.25 * MagicStateCultivation.EXPANDED_OPACITY))
    ...
    def append_expansion(self):
        ...
        for mmt in self.target.MOMENTS:
            self.target.append_expansion(self.circuitry, moment=mmt, prefix=f"EXP")
            self.circuitry.append_tick()

    def annotate_detectors(self, sdc_rounds: int, tpt_rounds: int):
        self.steane.annotate_detectors(self.circuitry, sdc_rounds=sdc_rounds, tpt_rounds=tpt_rounds)
        self.__junction.annotate_detectors(self.circuitry, rounds=tpt_rounds)
        self.source.annotate_detectors(self.circuitry, rounds=tpt_rounds + 1, prepared=Pauli.X)
        self.target.annotate_detectors(self.circuitry, sc_rounds=tpt_rounds + 1, source=self.source)
