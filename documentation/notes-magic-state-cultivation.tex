...
\begin{document}
	...
	The preparation stage is broken down into three main parts as shown in the figure below. In the \textsc{Injection} part, an attempt is made to prepare a logical magic state using a modified Steane Code circuit where a phase of $e^{i\pi/2}$ (resp. $e^{i\pi/4}$) is injected into a logical $\kett{+}_{\scriptscriptstyle L}$ state. \\
	...
	The \textsc{Steane Check} involves measuring the stabilisers of the Steane Code in order to determine whether the state lies in the correct code-space or has been corrupted. The \textsc{Phase Check} involves measuring a \Op{Y} stabiliser (resp. $\Op{H}_{\scriptscriptstyle XY}$) to confirm that the state has the correct relative phase between the states $\kett{0}_{\scriptscriptstyle L}$ and $\kett{1}_{\scriptscriptstyle L}$.
	...
	In the absence of errors, the seven data-qubits are in a logical state $\kett{+}_{\scriptscriptstyle L}$ once the red line is reached
	...
	As pointed out by Austin, the first two \Op{CNOT} gates after the red line result in all the physical states of the logical $\kett{0}_{\scriptscriptstyle L}$ (resp. $\kett{1}_{\scriptscriptstyle L}$) having qubit $7$ set to $\kett{0}$ (resp. $\kett{1}$). Once the $e^{i\pi/4}$ phase has been injected by the $\Op{T}^{\dagger}$ gate, the last two \Op{CNOT} gates change the value of qubit $7$ as to return the seven qubits to the state $\kett{+}_{\scriptscriptstyle L}$. See Figure (\ref{fig:injection-states}) for the detailed superpositions. \\
	...
	\paragraph{Preparation of $\kett{+}_{\scriptscriptstyle L}$ } Sticking to our assumed numbering, the stabiliser flows propagate up to the red line as follows
	...
	The product of the four X-stabiliser flows tells us we have prepared the +1 eigenstate of the observable $\Op{X}_{\scriptscriptstyle L}$
	\begin{equation*}
		\Op{X}_{1}\Op{X}_{3}\Op{X}_{5}\Op{X}_{7} \rightarrow \Op{X}_{124}\Op{X}_{2346}\Op{X}_{456}\Op{X}_{267} = \Op{X}_{1234567} = \Op{X}_{\scriptscriptstyle L}
	\end{equation*}

	\paragraph{Injection of $e^{\pi/2}$ phase}

	We need to prove that after the last five steps, assuming we use an $\Op{S}^{\dagger}$ gate instead of the $\Op{T}^{\dagger}$, we find that we have prepared the $+1$ eigenstate of the observable $\Op{Y}_{\scriptscriptstyle L}$

	\begin{equation*}
		\Op{X}_{1234567}\Op{Z}_{1345} \rightarrow \Op{Y}_{1234567} = \Op{Y}_{\scriptscriptstyle L}
	\end{equation*}
	...
	\begin{figure}[h!]
		...
		\begin{quantikz}[row sep={between origins, 0.75cm}, column sep={between origins, 0.75cm}]
			\lstick{$q_{\scriptscriptstyle 3}$} &&&\targ{}&&&&\ctrl{2}{}&& \\
			\lstick{$q_{\scriptscriptstyle 5}$} &&\targ{}&\phantom{i}&&&\ctrl{1}{}&\phantom{i}&& \\
			\lstick{$\kett{+}$}
			& \ctrl{1} &\ctrl{-1}&\ctrl{-2}&&&\targ{}&\targ{}&\ctrl{1}& \meter{\scalebox{0.5}{\Op{X}}} \\
			\lstick{$\kett{0}$}
			& \targ{} &\ctrl{2}&\ctrl{1}&&&\targ{}&\targ{}&\targ{}& \meter{} \\
			\lstick{$q_{\scriptscriptstyle 6}$} &&\phantom{i}&\targ{}&&&\phantom{i}&\ctrl{-1}&& \\
			\lstick{$q_{\scriptscriptstyle 7}$} &&\targ{}&&&&\ctrl{-2}&&&
		\end{quantikz}
	...
	\begin{figure}[h!]
		\centering
		\begin{quantikz}[row sep={between origins, 0.75cm}]
			\lstick{$q_{\scriptscriptstyle 6}$} & \gate[7,style={rounded corners=5pt}][1cm]{\Op{T}^\dagger}
			& \gate[7,style={rounded corners=5pt}][3cm]{\textsc{Cultivation}}
			& \gate[7,style={rounded corners=5pt}][1cm]{\Op{T}}
			& \\
			\lstick{$q_{\scriptscriptstyle 7}$} &&&& \\
			\lstick{$q_{\scriptscriptstyle 2}$} &&&& \\
			\lstick{$q_{\scriptscriptstyle 3}$} &&&& \\
			\lstick{$q_{\scriptscriptstyle 1}$} &&&& \\
			\lstick{$q_{\scriptscriptstyle 5}$} &&&& \\
			\lstick{$q_{\scriptscriptstyle 4}$} &&&&
		\end{quantikz}
	...
	\begin{figure}[h!]
		...
		\begin{quantikz}[row sep={0.85cm, between origins}, column sep={0.3cm}]
			...
			& &&&\targ{}&&\ctrl{-1} & \meter{\scalebox{0.5}{\Op{X}}} &\swtn&
			\\
			% Data qubit 7
			...
			& &&\phantom{i}&&&\ctrl{-1}& \meter{\scalebox{0.5}{\Op{X}}} & \swtn &
			\\
			% Data qubit 2
			...
			& &&\phantom{i}&\targ{}&&\ctrl{-1}& \meter{\scalebox{0.5}{\Op{X}}} &\swtn&
			\\
			% Data qubit 3
			...
			& &\targ{}&\ctrl{2}&&\ctrl{-1}&\ctrl{1}& \meter{\scalebox{0.5}{\Op{X}}} &\swtn&
			\\
			% Data qubit 5
...
%				\foreach \x in {0, 2, 4, 6, 8}{ \node[data] at (\x, -6) {\phantom{o}}; }
	...
	There are two observables that are going to be moved into the target Surface Code in the teleportation stage; $\Op{X}_{\scriptscriptstyle 267}$ and $\Op{Z}_{\scriptscriptstyle 124}$.
	...
	\begin{figure}[h!]
		...
		\begin{tabular}{|c|c|c|c|c|}
			...
			$\Op{X}_{\scriptscriptstyle 1237}$ & n/a & n/a & Destructive & Post-selection of $\Op{X}_{\scriptscriptstyle 1237ab}$ \\
			$\Op{Z}_{\scriptscriptstyle 1237}$ & Extended & Extended & n/a & Post-selection \\
			$\Op{X}_{\scriptscriptstyle 1345}$ & Superdense & n/a & Destructive & Post-selection \\
			$\Op{Z}_{\scriptscriptstyle 1345}$ & Superdense & Extended & n/a & Post-selection \\
			$\Op{X}_{\scriptscriptstyle 3567}$ & Superdense & n/a & Destructive & Post-selection \\
			$\Op{Z}_{\scriptscriptstyle 3567}$ & Superdense & Extended & n/a & Post-selection \\
			$\Op{X}_{ab}$ & n/a & n/a & Syndrome & Post-selection of $\Op{X}_{\scriptscriptstyle 1237ab}$ \\
			\hline
		\end{tabular}
		\caption{Summary of measurements involved in the Steane Code}
	\end{figure}

	In order to turn the logical operator $\Op{Z}_{\scriptscriptstyle 124}$ into $\Op{Z}_{\mbox{\scriptsize\tt abcde}}$, we measure the following stabilizers and the parity of their three measurement results gives us the sign relationship between these two logical operators

	\begin{equation*}
		p_{\scriptscriptstyle \Op{Z}} ~=~ (-1)^{\bar{\Op{Z}}_{2\tt a}}(-1)^{\bar{\Op{Z}}_{14\tt bc}}(-1)^{\bar{\Op{Z}}_{\tt de}}
	\end{equation*}

	In order to turn the logical operator $\Op{X}_{\scriptscriptstyle 267}$ into $\Op{X}_{\scriptstyle\tt afghi}$ ({\tt fghi} are the data qubits vertically aligned below {\tt a}), we simply use the parity of the three results of the destructive measurements on qubits $2$, $6$ and $7$ to determine the sign relationship between these two logical operators.
