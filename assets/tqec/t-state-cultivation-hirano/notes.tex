	...
	\begin{figure}[h!]
		...
		\begin{subfigure}[t]{0.62\textwidth}
			...
			\begin{quantikz}[row sep={0.75cm, between origins}, column sep={0.3cm}]
				% Data qubits
				\lstick{$q_{2}$} & \gate{\Op{T}^\dagger} &\targ{} &&&&& \\
				& \midstick{$\kett{+}$}\wireoverride{n} & \ctrl{-1} &\targ{}&&&& \\
				\lstick{$q_{3}$} & \gate{\Op{T}^\dagger} &\targ{} &\ctrl{-1}&&\ctrl{10}&\ctrl{5}& \meter{} \\
				& \midstick{$\kett{+}$}\wireoverride{n} & \ctrl{-1} &&&\phantom{i}&\phantom{i}& \\
				\lstick{$q_{4}$} & \gate{\Op{T}^\dagger} &\targ{} &&&\phantom{i}&\phantom{i}& \\
				& \midstick{$\kett{+}$}\wireoverride{n} & \ctrl{-1} &\targ{}&&\phantom{i}&\phantom{i}& \\
				\lstick{$q_{5}$} & \gate{\Op{T}^\dagger} &\targ{} &\phantom{i}&&\phantom{i}&\phantom{i}& \\
				& \midstick{$\kett{+}$}\wireoverride{n} & \ctrl{-1} &\ctrl{-2}&\ctrl{1}&\phantom{i}&\targ{}& \\
				\lstick{$q_{1}$} & \gate{\Op{T}^\dagger} &&&\targ{}&&& \\
				\lstick{$q_{6}$} & \gate{\Op{T}^\dagger} &\targ{} &&&\phantom{i}&& \\
				& \midstick{$\kett{+}$}\wireoverride{n} & \ctrl{-1} & \targ{} &&\phantom{i}&& \\
				& \midstick{$\kett{+}$}\wireoverride{n} & \ctrl{1} &\phantom{i}&&&& \\
				\lstick{$q_{7}$} & \gate{\Op{T}^\dagger} &\targ{} & \ctrl{-2} &&\targ{}&& \\
			\end{quantikz}
