...
def vertices(junction, row, col):
    ...
    if 0 <= ptype <= 7:
        ...
        vm = [(0.5, 0.5), (1.5, 0.5), (1.0, 0.0), (1.0, 1.0)]
    elif ptype == 14:
    ...
    for row, col in extended_stabilizers:
        if moment == 0:
            if forward:
                ...
                targets_rx.append(mq_at_locations[col + 0.5, row + 1.5])
                targets_rz.append(mq_at_locations[col + 1.0, row + 1.0])
        elif 1 <= moment <= 6:
            if moment == 1:
                if forward:
                    ...
                    targets_cx.append(mq_at_locations[col + 1.0, row + 1.0])
                    targets_rz.append(mq_at_locations[col + 0.5, row + 0.5])
            elif moment == 2:
                if forward:
                    targets_cx.append(mq_at_locations[col + 0.0, row + 1.0])
                    targets_cx.append(mq_at_locations[col + 0.5, row + 1.5])
...
if __name__ == "__main__":
    ...
    circuit_file = "../assets/tqec-extended-stabilizers-issue578.stim"
    ...
    with open("../assets/tqec-extended-stabilizers-issue578.stim", "w", encoding="utf-8") as file:
