...
def produce_template() -> np.ndarray:
    ...
    for r in [0, 1, 2, 3, 12, 13, 14, 15]:
        if r % 2 == 0:
            ...
            template[r + 1, 11] = 9
    template[7, 0] = template[9, 0] = 19
    template[8, 17] = template[10, 17] = 17
    ...
    for c in [0, 1, 2, 3, 4, 11, 12, 13, 14, 15]:
        if c % 2 == 0:
            ...
            template[6, c + 1] = 8
    template[0, 8] = template[0, 10] = 16
    template[17, 7] = template[17, 9] = 18
...
def append_regular_stabilizers(
    circuit, junction, mq_at_locations, dq_at_locations, moment, direction
):
    regular_stabilizers = filter(
        lambda position: (
            0 <= junction[*position] <= 11 or 16 <= junction[*position] <= 19
        ),
    ...
    for row, col in regular_stabilizers:
        ...
        if moment == 0:
            ...
            try:
                ...
                print(f"{ptype} @ {row, col}")
                raise ke
        elif moment == 6:
            if (ptype == 17 and direction == "forward") or (
                ptype == 19 and direction == "reverse"
            ):
                vertices_d = [(0, 0), (1, 0), (0, 1), (1, 1)]
...
def append_extended_stabilizers(
    circuit, junction, mq_at_locations, dq_at_locations_dq, moment, direction
):
...
def print_schedules():
    for ptype in all_schedules["forward"].keys():
        ...
        print(
            f"{plaquettes[encoding[ptype]]} [{hex(ptype)[2:]}] :\n{pretty_schedules(forward, reverse)}"
        )
...
if __name__ == "__main__":
    ...
    circuit_file = "../assets/tqec-extended-stabilizers/tqec-extended-stabilizers-flipping-temporal.stim"
    ...
    with open(circuit_file, "r", encoding="utf-8") as file:
        ...
        for row, col in itertools.product(range(SIDE), range(SIDE)):
            ...
            plaquette_type = plaquettes[encoding[stabilizer_type]][0]
    ...
    with open(
        "../assets/tqec-extended-stabilizers/tqec-extended-stabilizers-flipping-temporal.stim",
        "w",
        encoding="utf-8",
    ) as file:
